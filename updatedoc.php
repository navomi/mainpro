...
<body>
    ...
    <fieldset>
                ...
                <legend > <h2>Update Document Details</h2></legend>
...
<br>
 ...
 <?php
            ...
            if(!$error)
            {
                   ...
                   global $conn;
                  if ($stmt = $conn->prepare("UPDATE `documents` SET  `name`=?, `upload`=?, `doc_date`=?, `doc_time`=?, `type`=?, `hospital`=? WHERE `doc_id`=$_edit ")) ;


                             {
                    $stmt->bind_param("ssssss", $name, $file, $doc_date, $doc_time, $type, $hospital);
                    $stmt->execute();
                    $result = $stmt->execute();
                }
