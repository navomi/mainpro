            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <legend > 
                            <h2>Blood Glucose </h2> ADD <a href="create_blood_glucose.php"><span class="icon-plus icon-2x" style="color: #b2c831; margin-left: 10px"></a>
                    ...
                    <?php
                            ...
                            {
                                global $conn;
                                if ($stmt = $conn->prepare("SELECT parameter_no, para_date, para_time, reading, type  FROM `tracking_para` where id = ? and para_id = 2 ORDER BY para_date ASC")) 
                                     {
                            ...
                            {
                                global $conn;
                                if ($stmt = $conn->prepare("SELECT parameter_no, para_date, para_time, reading, type  FROM `tracking_para` where id = ? and para_id = 2 and MONTH(para_date) = 1 ORDER BY para_date ASC")) 
                                     {
                                        $stmt->bind_param("i", $id);
                                        $stmt->execute();
                                        $stmt->bind_result($parameter_no, $para_date, $para_time, $reading, $type);
                            ...
                            {
                                global $conn;
                                if ($stmt = $conn->prepare("SELECT parameter_no, para_date, para_time, reading, type  FROM `tracking_para` where id = ? and para_id = 2 and MONTH(para_date) = 2 ORDER BY para_date ASC")) 
                                     {
                                        $stmt->bind_param("i", $id);
                                        $stmt->execute();
                                        $stmt->bind_result($parameter_no, $para_date, $para_time, $reading, $type);
                            ...
                            {
                                global $conn;
                                if ($stmt = $conn->prepare("SELECT parameter_no, para_date, para_time, reading, type  FROM `tracking_para` where id = ? and para_id = 2 and MONTH(para_date) = 3 ORDER BY para_date ASC")) 
                                     {
                                        $stmt->bind_param("i", $id);
                                        $stmt->execute();
                                        $stmt->bind_result($parameter_no, $para_date, $para_time, $reading, $type);
                            ...
                            {
                                global $conn;
                                if ($stmt = $conn->prepare("SELECT parameter_no, para_date, para_time, reading, type  FROM `tracking_para` where id = ? and para_id = 2 and MONTH(para_date) = 4 ORDER BY para_date ASC")) 
                                     {
                                        $stmt->bind_param("i", $id);
                                        $stmt->execute();
                                        $stmt->bind_result($parameter_no, $para_date, $para_time, $reading, $type);
                            ...
                            {
                                global $conn;
                                if ($stmt = $conn->prepare("SELECT parameter_no, para_date, para_time, reading, type  FROM `tracking_para` where id = ? and para_id = 2 and MONTH(para_date) = 5 ORDER BY para_date ASC")) 
                                     {
                                        $stmt->bind_param("i", $id);
                                        $stmt->execute();
                                        $stmt->bind_result($parameter_no, $para_date, $para_time, $reading, $type);
                            ...
                            {
                                global $conn;
                                if ($stmt = $conn->prepare("SELECT parameter_no, para_date, para_time, reading, type  FROM `tracking_para` where id = ? and para_id = 2 and MONTH(para_date) = 6 ORDER BY para_date ASC")) 
                                     {
                                        $stmt->bind_param("i", $id);
                                        $stmt->execute();
                                        $stmt->bind_result($parameter_no, $para_date, $para_time, $reading, $type);
                            ...
                            {
                                global $conn;
                                if ($stmt = $conn->prepare("SELECT parameter_no, para_date, para_time, reading, type  FROM `tracking_para` where id = ? and para_id = 2 and MONTH(para_date) = 7 ORDER BY para_date ASC")) 
                                     {
                                        $stmt->bind_param("i", $id);
                                        $stmt->execute();
                                        $stmt->bind_result($parameter_no, $para_date, $para_time, $reading, $type);
                            ...
                            {
                                global $conn;
                                if ($stmt = $conn->prepare("SELECT parameter_no, para_date, para_time, reading, type  FROM `tracking_para` where id = ? and para_id = 2 and MONTH(para_date) = 8 ORDER BY para_date ASC")) 
                                     {
                                        $stmt->bind_param("i", $id);
                                        $stmt->execute();
                                        $stmt->bind_result($parameter_no, $para_date, $para_time, $reading, $type);
                            ...
                            {
                                global $conn;
                                if ($stmt = $conn->prepare("SELECT parameter_no, para_date, para_time, reading, type  FROM `tracking_para` where id = ? and para_id = 2 and MONTH(para_date) = 9 ORDER BY para_date ASC")) 
                                     {
                                        $stmt->bind_param("i", $id);
                                        $stmt->execute();
                                        $stmt->bind_result($parameter_no, $para_date, $para_time, $reading, $type);
                            ...
                            {
                                global $conn;
                                if ($stmt = $conn->prepare("SELECT parameter_no, para_date, para_time, reading, type  FROM `tracking_para` where id = ? and para_id = 2 and MONTH(para_date) = 10 ORDER BY para_date ASC")) 
                                     {
                                        $stmt->bind_param("i", $id);
                                        $stmt->execute();
                                        $stmt->bind_result($parameter_no, $para_date, $para_time, $reading, $type);
                            ...
                            {
                                global $conn;
                                if ($stmt = $conn->prepare("SELECT parameter_no, para_date, para_time, reading, type  FROM `tracking_para` where id = ? and para_id = 2 and MONTH(para_date) = 11 ORDER BY para_date ASC")) 
                                     {
                                        $stmt->bind_param("i", $id);
                                        $stmt->execute();
                                        $stmt->bind_result($parameter_no, $para_date, $para_time, $reading, $type);
                            ...
                            {
                                global $conn;
                                if ($stmt = $conn->prepare("SELECT parameter_no, para_date, para_time, reading, type  FROM `tracking_para` where id = ? and para_id = 2 and MONTH(para_date) = 12 ORDER BY para_date ASC")) 
                                     {
                                        $stmt->bind_param("i", $id);
                                        $stmt->execute();
                                        $stmt->bind_result($parameter_no, $para_date, $para_time, $reading, $type);
             ...
             # code...
                            ...
                            <script>
                                ...
                                // load the data
                                var chartData = AmCharts.loadJSON('chartdata_glucose.php');
